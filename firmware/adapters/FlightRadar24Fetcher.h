...
so this fills the inline-enrichment fields on StateVector. A row that includes a
route lets the Area-mode path skip the per-flight adsbdb/hexdb lookup entirely;
type and airline ride along regardless of whether a route was present, and are
layered onto that lookup's result when it does run. The one thing the feed does
not carry is a readable airline NAME — it gives the operator as an ICAO code — so
that is resolved on device from utils/AirlineNames.h.
