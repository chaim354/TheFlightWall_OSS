...
**Silent misconfiguration has no web-UI signal — flagged, not fixed here.** If `serverUrl` is empty, wrong, or unreachable, `fetchServerMode` falls back to adsb.lol and the wall keeps working; that fail-open behaviour is by design and is exactly what Task 9 Step 4 verifies. But nothing in the web UI shows the fallback happened: `ok` stays true (the adsb.lol leg of the fallback sets it), the status note is the ordinary `"area mode"` success text, and `serverStale` only ever reflects a *reachable* server's own cache state — every early-return path in `FlightWallServerFetcher::fetchFlights`, including all three misconfiguration shapes, leaves it at its default `false` rather than signalling "not contacted." A user who typos their server URL gets a wall that keeps working and silently never uses it. Not a total blind spot: `fetchServerMode` already logs `"server unavailable; falling back to adsb.lol"` every such cycle, so the diagnostic exists today for anyone watching the serial console — this gap is specifically in the web UI's status pill, which has no state for "fell back." A real fix needs a signal `FlightDataFetcher` does not currently expose: something that distinguishes "server contacted and used this cycle" from "fell back this cycle." `lastFetchStale()` cannot be stretched to cover it — by contract it only describes a successful server response's own staleness, not whether the server was reached at all. That signal is beyond what this plan scoped.
