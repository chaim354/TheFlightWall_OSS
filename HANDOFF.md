...
| Work | Status |
|---|---|
| **Unified dashboards + airline ignore list** (2026-09-09, merged to `main` as `485c2c2`) | **SERVER DEPLOYED AND LIVE. FIRMWARE `485c2c2` DEVICE-VERIFIED ON A SECOND S3 -- BUT THE WALL WENT SILENT COLLECTING IT; read the row below before anything else.** Both pages now carry every control except WiFi: the remote page gained the Flights-mode list, both airline lists, the server URL (admin), HUB75 presets and a `clearui` action; the LAN page gained the enrichment cache and a "FlightWall server" card that drives watched flights and airline names cross-origin (the server answers CORS on `/v1/tracked*` and `/v1/airlines*` only). `filters.airlineDenyList` hides an operator on every source, is exempt for pinned flights, and travels to `/v1/flights` as `deny_airlines=` (the allow-list now travels too, as `allow_airlines=`) so the server's nearest-N cut respects it. Verified on a second ESP32-S3 N16R8 flashed over USB with the same image the wall downloaded: boots, PSRAM 8.35MB, joins WiFi, fetches, `set {"filters":{"airlineDenyList":["eja"]}}` over serial persists as `["EJA"]` and the very next `/v1/flights` request carried `deny_airlines=EJA,NJE` (proxy logs), `/api/flights` carries `operatorIcao`, `POST /api/updateui` pulled the published page and served it, and that page drove the production server cross-origin from a real device origin. Also: 713 server tests, 15 host suites, both envs build, the Unity suite compiles. Spec: `docs/superpowers/specs/2026-09-09-unified-dashboards-airline-ignore-design.md`. **Ship order matters:** firmware before the web UI (`updatefw`, then `updateui`), or the new page posts two keys the old firmware drops silently. |
| **THE WALL WENT SILENT AFTER COLLECTING `updatefw` (2026-09-09, 16:09Z)** | **UNRESOLVED AT HAND-OFF, and NOT the image's fault by the evidence above.** Proxy logs (`kamal proxy logs`, filter on `flightwall.tinkerex.com` + `ESP32HTTPClient` -- NOT on the user agent alone, which also matches the transit display on the same box) show the wall at `173.56.3.154` fetching and checking in every 30s on `c2f04f9`, then at 16:09:00Z a check-in that drained `updatefw`, 16:09:01Z the manifest, 16:09:02Z `GET /assets/firmware/firmware.bin` 200 (1224048 bytes; Cloudflare buffers, so the 41ms says nothing about how long the device took), and **nothing since** -- no fetch on either image, 35+ minutes at hand-off. With rollback a new image that crashed would have handed the old one back within minutes and it would have checked in, so the silence is the REBOOT not coming back, which on this board has meant power (see "The board drops off USB AND WiFi together"). The maintainer's Mac was on a DIFFERENT network (public IP 74.101.244.108 vs the wall's), so every LAN probe from it was meaningless. **Next: someone at the wall looks at the panel -- dark, frozen, or showing an IP/`FlightWall-Setup` -- then power-cycles it.** `updateui` is queued and harmless on either image; `updatefw` was NOT re-queued (at-most-once is spent, and a second download is the maintainer's call if the first one is what killed it). The control page's status card now says which firmware the server offers versus what the wall runs, so the outcome is readable from there the moment it checks in. |
...
| Idle-when-dark + server quiet hours (`14e0647`..`01ae579`), merged as `eec3cb2` | **DEVICE-VERIFIED, 2026-08-24.** Flashed APP-ONLY (`-t upload`, deliberately NOT `uploadfs`) -- settings survived, device rejoined on its own. Gate confirmed on hardware: brightness 0 -> `/api/status` note `panel dark - fetch paused`, held across a full 60s interval with the 12 held flights RETAINED (70s < the 360s stale window), then `fetch ok` within one cycle of restoring brightness. Server deployed (`eec3cb2f09ea`, healthy) with `REFRESH_QUIET_HOURS=0-6` live; its boot log `schedule: 3144 rows from 4/4 boards` IS the cold-start refresh added this session. NOT exercised: the stale-discard path (needs >6min dark) and the quiet window itself (needs 00:00-06:00 ET). **THE "INERT AS CONFIGURED" NOTE BELOW IS NO LONGER TRUE -- corrected 2026-08-25.** The live device reports `schedule.enabled=true`, `nightBrightness=0`, `light.enabled=true`, `dimInstead=false`. Both paths to brightness 0 are now ARMED, so the dark gate can and will halt fetching. It read `lightLevel` 85 at 19:13Z and 70 at 19:28Z against `darkThreshold=50` -- drifting toward the trip point, with `hysteresis=150` meaning recovery needs >200, which a sensor reading 70 in daylight will not reach. The maintainer was shown this on 2026-08-25 and chose to leave it armed; that is a decision, not an oversight. If the wall goes BLANK and stops updating, this is the first thing to check. (Original note, now false: "the device half is INERT as configured -- schedule.enabled=false, nightBrightness=5, light.enabled=false mean nothing reaches brightness 0".) |
...
| Board reliability scare, 2026-08-25 | **NOT A FAULT -- it was physical handling, and the measurements are worth keeping.** Symptom: the wall stopped updating. Traced to the maintainer moving the ESP32 onto the panel's 5V rail and unplugging USB: 32 flawless 60s fetches until 18:44:26Z when handling began, the "reboot" at 18:58:21Z was the power swap itself (uptime arithmetic, not a brownout), 12m silence while work continued, then a locked 60s cadence from 19:11:31Z onward. **Ping, both 100 packets in the same window: device 0.0% loss 5.414/60.134/379.183/62.488ms, router 0.0% loss 1.946/3.070/13.452/1.350ms.** So NOT the §1 radio degradation (that signature is 11-35% loss, ~528ms avg) -- but 60ms avg against the 7.2ms measured after a good reseat is 8x, on the axis that has degraded twice before. **Remote diagnosis recipe, which did not exist before and beats guessing:** `kamal proxy logs` filtered on `req_user_agent=ESP32HTTPClient` gives the true fetch cadence and every gap; `kamal app exec --reuse 'cat /app/data/control.json'` gives `uptimeS`, which is what distinguishes a REBOOT from a device that is up but silent; and the device's own `http://<ip>/api/status` gives `lightLevel`/`lightDark`, which is what distinguishes the dark gate from a link failure. A dark-gated panel is BLANK; a link failure shows the no-flights dots. That distinction is what refuted the first hypothesis here. |
| **OTA was broken, and is now fixed and PROVEN** (`1031a53`, `8fee649`) | **DEVICE-VERIFIED, 2026-08-25 -- an over-the-air update completed where three in a row had failed.** Symptom: `updatefw` did nothing, three times, leaving the wall on its old image. Ruled out by elimination BEFORE touching anything: signing key (unchanged since `a607c6e`, predating the running image), OTA slot size (2x3MB vs a 1.2MB image), and the server (200 with the exact byte count, every attempt). A USB serial line finally gave the whole diagnosis in one string: **`download failed (-11)` = `HTTPC_ERROR_READ_TIMEOUT`**, against the 30s `setTimeout` already in force -- so the connection STALLED for over half a minute mid-stream. **Cause: the panel's I2S DMA starving the radio under SUSTAINED load.** The trap is that a 1-per-second ping read 4% loss in the same window and looked survivable; a ping barely loads the radio, a 1.2MB stream saturates it. **Fix: `g_display.stopOutput()` for the duration of the download**, reusing exactly what AP setup mode already does ("wifi will struggle until it is dark again"). It halts DMA, not brightness, so it does NOT reach `decideIdle()` -- suppressing fetches during an OTA would stop the check-ins that deliver it. Server log, one line per attempt: `20:29:24` fail, `20:34:24` fail, `20:40:24` fail, **`20:49:38` SUCCESS** on the first build carrying the fix. `8fee649` also puts the OTA error in `/api/status` and the check-in, so the next failure is readable from the control page instead of requiring a cable and an afternoon. |
| Ribbon degradation, THIRD occurrence (2026-08-25) | **DIAGNOSED AND RESEATED.** The wall stopped updating after the maintainer moved the ESP32 onto the panel's 5V rail. Measured, paired with a router control in every window (router 0.0% / ~3ms throughout, so never the LAN): healthy earlier that afternoon **0.0% / 38ms**; degraded **13.6% / 806ms**, then **39.0% / 1231ms**, then **45.0% / 754ms** -- the worst this fault has ever measured here. **USB was NOT the variable** (45% with it in, 39% with it out), which is what refuted the power hypothesis and pointed back at mechanical retention. After reseating: **4.0% / 458ms**, RSSI -61 -> -45. Note 4% is BETTER but NOT the 0.0% / 7.2ms a good reseat produced in 2026-08-24 -- and 4% was still enough to make a 1.2MB OTA impossible until the row above was fixed. **Three degradations, all shortly after physical handling.** Treat the connector's retention as the suspect, not the seating technique. |
| **NOT the ribbon -- POWER.** The 2026-08-25 room-move outage | **NOT RESOLVED. The "power" conclusion recorded here was WRONG, and the method that produced it was the real mistake.** It rested on ONE before/after pair (USB out 84.7% loss, USB in 0.0%) -- and later the same evening, with USB never unplugged, the link measured **86.7% loss**, then **30.0%** after a soft reboot, router 0.0% throughout. Every reading at that location: 0.0 / 30 / 39 / 85 / 87 percent. **This fault swings ~32x between identical runs (see section 1), so a single before/after pair establishes NOTHING**, and two causal claims were built on exactly that -- first the ribbon, then the power -- and both were contradicted within hours. What IS established: the board's radio is severely and intermittently degraded, at BOTH locations, across power arrangements, after reseating, and after a reboot, while a router control answers 0.0% every time. A soft reboot improved it (86.7 -> 30) without fixing it, which fits neither a purely physical fault nor a purely software one. **Next step is a CONTROLLED experiment, not another guess:** multiple multi-minute runs per condition with the panel's DMA stopped versus running (g_display.stopOutput() now exists and is exercised by the OTA path), which is the one mechanism section 1 measured as real and which nobody has ever tested properly. Measured at that location minutes apart: **USB out 84.7% loss / 1081ms avg; USB in 0.0% loss / 123ms avg.** Everything else was a wrong turn, and the wrong turns are the useful part: (1) RSSI read -48 to -56 throughout and is WORTHLESS here -- it measures signal strength, not supply sag; (2) the ribbon was reseated and did NOT fix it (86.7% -> 65%), which should have been taken as evidence against the ribbon sooner; (3) a router control ping was run from a 5GHz Mac while the ESP32 is 2.4GHz-only, so it excluded nothing, and a 40MHz-wide neighbour AP on channel 5 looked like a plausible culprit it was not; (4) loss was bursty and irregular, which was read as external interference and is equally consistent with a supply browning out under TX bursts. **THE OLD LOCATION'S FAULT WAS GENUINELY DIFFERENT:** there USB made no measurable difference (45% vs 39%) and reseating the ribbon took 45% -> 4%. Two faults, one symptom -- a starved radio -- and carrying the ribbon diagnosis across a move that changed the power arrangement is what cost the evening. **Diagnostic order for next time: plug in USB FIRST.** If it recovers, it is power; if it does not, then look at the ribbon. It is a ten-second test that separates the two. Durable fix here is a clean dedicated 5V feed for the ESP32 (its own 2A+ supply, or short/thick leads plus 470-1000uF of bulk capacitance at VIN), not the panel rail alone. |
| **The board drops off USB AND WiFi together -- suspect POWER, not RF** (2026-08-25, 23:28Z) | **The most diagnostic event of a long evening, and it arrived last.** During a controlled panel-DMA experiment the board went to 100% packet loss on every run regardless of condition, then stopped enumerating over USB entirely: no `/dev/cu.usbmodem*`, no LAN reply, no server contact, while a router control answered 0.0% throughout. **Nothing about RF can take a device off a USB bus.** That rules out interference, antenna placement, channel width and ribbon seating in one observation -- all the things the evening was spent on. Section 0 records the same symptom once before ("the board dropped off USB before it could be flashed"), so this is not a one-off. A supply browning out under load explains everything that defeated diagnosis: swings between 0% and 100% loss, fixes that appear to work then stop (a ribbon reseat, then USB power -- BOTH were later contradicted), RSSI reading -46 to -56 throughout because the radio reports healthy right up until it is unpowered, and reboots that help briefly. **Diagnostic order for the next occurrence: (1) is the panel lit, dark or frozen -- that separates a dead board from a crashed ESP32 with the panel still powered; (2) does USB re-enumerate on replug -- yes means a crash, no means no power reaching it; (3) measure 5V AT THE ESP32 VIN UNDER LOAD, which is the one joint nobody checked all evening.** Do NOT start with ping tests: they measure the victim, not the cause. |
| **MatrixPortal S3: plugging it into the panel costs ~20dB of RSSI** (2026-08-25) | **MEASURED on the board itself, same firmware, minutes apart: RSSI -55/-56 with `fetch ok` and flights held while the panel was UNPLUGGED; -74 to -80 with `fetch failed`, 0 flights and intermittent HTTP the moment the panel was connected.** The cause is mechanical rather than electrical noise: this board mounts DIRECTLY onto the HUB75 socket, so its PCB antenna ends up flat against the panel's copper ground plane, which detunes and attenuates it. The DevKit is not exposed to this in the same way because it hangs off flying leads with air around it. **Consequences: (1) the onboard-antenna variant (ADA5778) is arguably WORSE than the DevKit for this build, not better; (2) the u.FL variant (ADA6475) is the one worth buying, because the antenna can be moved off the panel entirely; (3) a free mitigation exists and should be tried first -- the board also carries a 2x8 IDC plug for a standard HUB75 RIBBON, so it can be mounted away from the panel instead of onto it.** Note this also means the board is a clean testbed for the coupling question the DevKit could never settle: plugged-in versus ribbon-mounted is a controlled A/B on one piece of hardware. |
...
`claude/audit-priority-list-d3dbf5`, which branched at `d58ed83`), the calendar
sync as `bdb7362`, and the OTA fix as `8fee649`. That is exactly what is flashed to the device and deployed to
