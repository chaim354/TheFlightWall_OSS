...
| Ribbon degradation, FOURTH occurrence (2026-08-25, after a room move) | **CORRECTION, same evening: the claim that the network was excluded was WRONG, and the reasoning behind it was flawed.** The router control was pinged from a Mac on **5GHz channel 157** while the ESP32 is **2.4GHz-only** -- so it tested a band the board does not use and excluded nothing. Reseating then FAILED to fix this occurrence (86.7% -> 65%, where the same action gave 45% -> 4% hours earlier), which is itself evidence against the ribbon being the cause here. The 2.4GHz environment at the new location carries a **40MHz-wide AP on channel 5**, overlapping channels 1-9 and colliding with neighbours on 1 and 6 -- a known cause of heavy loss at strong RSSI. UNRESOLVED as of this writing; the two candidates (board-side DMA/ribbon vs 2.4GHz congestion) have NOT been separated. To separate them: set the router's 2.4GHz to 20MHz width on channel 1 or 11 and re-measure, or move the board back to the previous network. **Do not ping a control from a 5GHz client and call it a control.** The wall was moved to another room and another network (`Fios-ZI09L`, 192.168.1.221) and stopped getting flights within the hour. Paired measurement from a Mac on that same new LAN: **board 86.7% loss / 1593ms avg, router 0.0% / 2.87ms** -- the worst reading this fault has ever produced here (previous worst 45%). Different router, different SSID, different room: the fault travelled with the BOARD, which no earlier occurrence could establish. **RSSI was -50 throughout and is worthless as an indicator** -- it measures signal strength, not DMA contention; -50 with 86.7% loss is the same pattern as -59 with 45% earlier the same day. Symptom presents as `note: fetch failed - showing last known`, `flightCount: 0`, because at that loss rate a TLS handshake cannot complete and BOTH the server and the adsb.lol fallback fail. **Four degradations, every one shortly after the device was physically handled.** Reseating restores service (45% -> 4% earlier that day) but is now demonstrably a TEMPORARY measure -- the connector is not retaining. The durable fix is mechanical: strain-relief so the ribbon's weight and movement are not carried by the connector, or a new ribbon/connector if the latches are worn. Anyone reseating for a fifth time without addressing retention is buying hours, not a fix. |
